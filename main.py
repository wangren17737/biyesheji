...
model.classifier[6] = nn.Linear(num_ftrs, len(emotion_labels))
...
# 训练并保存模型模型
#model.load_state_dict(torch.load('vgg19_emotion.pth'))
model = train_model(model, criterion, optimizer, exp_lr_scheduler, num_epochs=50)
torch.save(model.state_dict(), 'vgg19_emotion.pth')
